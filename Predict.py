...
def AddOdd(m, o, d):
    m.machineWinHome += o.hw * d
    m.machineWinAway += o.aw * d


def PythagoreanExpectation(hometeam, awayteam):
    hometeamt = pow(hometeam.totalGoals, 1.22777) / (
            pow(hometeam.totalGoals, 1.0723880529403687) + pow(hometeam.totalConcede,
                                                               1.1272480487823486)) * 2.4999730587005615 * hometeam.totalMatch
    awayteamt = pow(awayteam.totalGoals, 1.22777) / (
            pow(awayteam.totalGoals, 1.0723880529403687) + pow(awayteam.totalConcede,
                                                               1.1272480487823486)) * 2.4999730587005615 * awayteam.totalMatch
...
def PythagoreanExpectationWithSides(hometeam, awayteam):
    hometeamt = pow(hometeam.homeGoals, 1.22777) / (
            pow(hometeam.homeGoals, 1.0723880529403687) + pow(hometeam.homeConcede,
                                                              1.1272480487823486)) * 2.4999730587005615 * hometeam.homeMatch
    awayteamt = pow(awayteam.awayGoals, 1.22777) / (
            pow(awayteam.awayGoals, 1.0723880529403687) + pow(awayteam.awayConcede,
                                                              1.1272480487823486)) * 2.4999730587005615 * awayteam.awayMatch
    total = hometeamt + awayteamt
    htr = hometeamt / total
    atr = awayteamt / total
    toReturn = Odd(0)
...
def calculateResultPerc(currentLeague, team1, team2):
    ...
    hometeam_DefenseStrength = (hometeam_HomeConceded / hometeam_HomeMatches) * (
            league_HomeConceded / league_TotalMatch)
    awayteam_DefenseStrength = (awayteam_AwayConceded / awayteam_AwayMatches) * (
            league_AwayConceded / league_TotalMatch)
    if (awayteam_AwayConceded == 0):
    ...
    toReturn.hw = (hometeam_PossibleGoals / (hometeam_PossibleGoals + awayteam_PossibleGoals))
    toReturn.aw = (awayteam_PossibleGoals / (hometeam_PossibleGoals + awayteam_PossibleGoals))
    return toReturn


def fillodds(m):
    m.machineWinHome = abs(m.machineWinHome)
    m.machineWinAway = abs(1 - m.machineWinHome)
    hw = m.machineWinHome / (m.machineWinHome + m.machineWinAway)
    aw = m.machineWinAway / (m.machineWinHome + m.machineWinAway)
    m.machineHomeRatio = hw
    m.machineAwayRatio = aw
    m.machineDrawRatio = abs(m.machineHomeRatio - m.machineAwayRatio)
    left = 1 - m.machineDrawRatio
    m.machineHomeRatio = left * hw / (hw + aw)
    m.machineAwayRatio = left * aw / (hw + aw)
    ...
    if (t1.totalMatch > 0 and t2.totalMatch > 0):
        if (t1.totalGoals > 0 and t2.totalGoals > 0 and t1.totalConcede > 0 and t2.totalConcede > 0):
            if (t1.homeGoals > 0 and t2.homeConcede > 0 and t1.awayConcede > 0 and t2.awayGoals > 0):
                AddOdd(m, PythagoreanExpectationWithSides(t1, t2), k.pyExSide)
                # print("PythagoreanExpectationWithSides",m.machineWinHome,m.machineWinAway)
                AddOdd(m, PythagoreanExpectation(t1, t2), k.pyEx)
                # print("PythagoreanExpectation",m.machineWinHome, m.machineWinAway)
                AddOdd(m, calculateResultPerc(l, t1, t2), k.resultPerc)
                # print("calculateResultPerc",m.machineWinHome, m.machineWinAway)
    # print(m.machineHomeRatio,m.machineDrawRatio,m.machineAwayRatio)
    AddOdd(m, PowerRank(t1, t2), k.powerRank)
    # print("PowerRank", m.machineWinHome, m.machineWinAway)
    fillodds(m)
    # print(t1.totalMatch > 0 and t2.totalMatch > 0,t1.totalGoals > 0 and t2.totalGoals > 0 and t1.totalConcede > 0 and t2.totalConcede > 0,t1.homeGoals > 0 and t2.homeConcede > 0 and t1.awayConcede > 0 and t2.awayGoals > 0)
    # print(m.machineHomeRatio,m.machineDrawRatio,m.machineAwayRatio)
    return m
